...
TMDB 5000 영화 데이터를 기반으로 만든 Streamlit 콘텐츠 기반 영화 추천 웹앱입니다. 사용자가 영화 한 편을 선택하면 기존 `cosine_sim2.pkl` 추천 모델을 우선 사용해 유사 영화 10편을 포스터와 함께 보여줍니다. `cosine_sim2.pkl` 로딩에 실패하면 `cosine_sim.pkl`로 자동 대체합니다.
